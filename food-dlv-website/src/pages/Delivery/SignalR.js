...
async function NotifyTheStore(storeId,orderId) {
    try {
      ...
      await connection.invoke("JoinGroup", { storeId, targetRole }); //連上StoreGroup
      await connection.invoke("NewOrder", { storeId, orderId }); //傳送訂單
      await connection.invoke("LeaveGroup", { storeId, targetRole }); //離開StoreGroup
...
async function LinkStart(memberId) {
    try {
      ...
      await connection.start();
      await connection.invoke("JoinGroup", { memberId, role });
    } catch (e) {
      console.log(e);
    }
  }
...
async function AssignOrder(driverId,orderId) {
    try {
      ...
      await connection.start();
      await connection.invoke("JoinGroup", { driverId, targetRole }); //連上driverGroup
      await connection.invoke("AssignOrder", { driverId, orderId }); //傳送訂單
      await connection.invoke("LeaveGroup", { driverId, targetRole }); //離開driverGroup
...
async function JoinGroup(storeId) {
    try {
      ...
      await connection.start();
      await connection.invoke("JoinGroup", { storeId, role });
    } catch (e) {
      console.log(e);
    }
  }
